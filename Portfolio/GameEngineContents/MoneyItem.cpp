...
void MoneyItem::SetType(MoneyType _Type, bool _IsSoundOn)
{
	...
	if (true == _IsSoundOn)
	{
		GameEngineSound::SoundPlayOneShotWithVolume("penny drop 1.wav", 0, 1.0f * Option_SFX);
	}
...
void MoneyItem::DestroyUpdate()
{
	if (Renderer_->IsEndAnimation())
	{
		GameEngineSound::SoundPlayOneShotWithVolume("penny pickup 1.wav", 0, 1.0f * Option_SFX);
		Off();
	}
}
