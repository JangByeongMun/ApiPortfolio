...
void KeyItem::SetType(KeyType _Type)
{
	...
	case KeyType::Normal:
		...
		GameEngineSound::SoundPlayOneShotWithVolume("keydrop.wav", 0, 0.5f * Option_SFX);
		break;
	case KeyType::Two:
...
void KeyItem::Update()
{
	...
	if (true == Collision_->IsUpdate() && true == Collision_->CollisionCheckRect("Player"))
	{
		...
		case KeyType::Normal:
			...
			GameEngineSound::SoundPlayOneShotWithVolume("keypickup.wav", 0, 0.5f * Option_SFX);
			break;
		case KeyType::Two:
		...
		case KeyType::Master:
			...
			GameEngineSound::SoundPlayOneShotWithVolume("golden key.wav", 0, 0.5f * Option_SFX);
			break;
		default:
			break;
		}
