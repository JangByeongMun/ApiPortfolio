...
void Mom::MonsterDamaged()
{
	if (HP_ <= 600 && false == IsHurt_[0])
	{
		IsHurt_[0] = true;
...
void Mom::LegStart()
{
	ResetValue();
	AnimTimer_ = 0.0f;
	GameEngineSound::SoundPlayOneShotWithVolume("evil laugh.wav", 0, 0.1f * Option_SFX);
...
void Mom::DeadStart()
{
	ResetValue();
	AnimTimer_ = 0.0f;
	GameEngineSound::SoundPlayOneShotWithVolume("vox death.wav", 0, 0.1f * Option_SFX);
}
