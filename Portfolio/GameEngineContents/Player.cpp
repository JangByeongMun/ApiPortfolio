...
void Player::Start()
{
	...
	if (false == GameEngineInput::GetInst()->IsKey("MoveLeft"))
	{
		...
		GameEngineInput::GetInst()->CreateKey("Test1", 0x31); // 1번
...
void Player::Update()
{
	...
	if (true == GameEngineInput::GetInst()->IsDown("Bomb"))
	{
		if (BombCount_ > 0)
		{
			...
			GameEngineSound::SoundPlayOneShotWithVolume("fetus land.wav", 0, 0.1f * Option_SFX);
		}
	}
...
void Player::ChangeRoom(DoorDir _Dir)
{
	...
	if (false == PlayLevel::GetDebugMode1or2())
	{
		if (0 != FindRoom->GetMonsterCount() || 0 != FindRoom->GetBossCount())
		{
			...
			GameEngineSound::SoundPlayOneShotWithVolume("door heavy close.wav", 0, 0.1f * Option_SFX);
		}
	}
